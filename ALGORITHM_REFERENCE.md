...
| `supabasePush()` | **Whole-row upsert** | `SUPA.saveLedger(uid, toGooglePayload())` — the entire ledger JSON is one payload written to the workspace's single `shared_ledgers` row. | O(n) serialize |
...
| `SUPA.saveLedger(uid, payload)` | **Upsert (snapshot storage)** | One shared row per workspace; the whole payload (`state` + `device` fact + `exportedAt`) is written in a single upsert. | O(n) |
| `SUPA.getLedger(uid)` | **Select-single** | Reads the row; distinguishes "no row" (`null` ⇒ confirmed empty) from errors. On an expired/invalid session it mirrors `saveLedger`'s heal: `getUser()` confirms the token server-side, and **one retry** follows a successful refresh — a device whose access token silently expired (a session supabase-js serves from cache) pulls the newest cloud copy on its very next refresh instead of freezing on the login-time snapshot. A dead refresh token surfaces as an honest \"session expired — sign in again\" while the 8s reconcile / 60s poll retry loops stay alive. | O(n) |
| `SUPA.subscribeRealtime(uid, cb)` | **Channel subscription** | Postgres-change channel filtered to the workspace row; each change hands the row to the callback (→ `supabaseUpdate`). | O(1) setup |
...
| `autoReportDone(pan)` / reset-cancel | **Once-only report + cancellation** | A finished pan reports its configured rolls into that day's production batch (quiet merge); manually resetting a finished pan cancels its pending count — nothing can ever be double-reported. Bags are never per-round: they derive from full sets (`rolls ÷ rollsPerBag`, floor) inside the ledger. | O(P + c) |
